...
The schema described here has been implemented for the deployed MVP database.

The current demo corpus uses:
- `cases`
- `clusters`
- `opinions`
- `chunks`
...
The deployed demo currently focuses on case/opinion/chunk retrieval. Citation table support exists in the schema and API shape, but full citation ingestion and richer related-case views are deferred beyond the current demo.
...
These are still open or future-facing and should be discussed explicitly when revising the schema.

### 6.1 Citation scope

**Status:** Schema supports this; full ingestion deferred for current demo

Question:
Should citations only store edges between opinions we have ingested, or also preserve references to out-of-corpus cases?

**Current schema direction:** support out-of-corpus references via nullable target IDs + raw reference fields.
...
**Current implementation note:** the `citations` table and case-detail API shape exist, but full citation ingestion and related-case presentation are not yet a focus of the deployed demo.

---

### 6.2 Cluster normalization depth

**Status:** Settled for MVP; future enrichment possible

Question:
Given that a normalized `clusters` table is now settled, how much should it contain for MVP?

**Current direction:** keep it thin.
...
**Status:** Deferred beyond current demo
...
**Current implementation:** `section_hint = NULL` for all chunks. The current chunker is paragraph-first and does not attempt legal section detection.
...
* chunks are derived from `opinions.clean_text`
* `section_hint` is currently `NULL`
* SQLite FTS5 indexes chunk text through `chunks_fts`
* FAISS maps vectors back to `chunks.id` through `storage/faiss/chunks_ids.npy`
...
* The current implementation uses paragraph-first chunking over `opinions.clean_text`.
* `section_hint` is currently set to `NULL`.
* Section-aware chunking is deferred until reliable heading/section detection is worthwhile.
...
* The `citations` table exists in the SQL schema.
* The case-detail API includes a `citations` field.
* Full citation ingestion and richer related-case presentation are deferred beyond the current deployed demo.
...
## 9. Verification Results and Current Implementation Status
...
The locked SQL schema has been wired into the application and current ingestion/chunking pipeline for the deployed demo corpus.

Current implementation:

* dockets normalize into `cases`
* clusters normalize into `clusters`
* opinions normalize into `opinions`
* `opinions.clean_text` is derived using the locked fallback priority
* chunks are created from `opinions.clean_text`
* SQLite FTS5 indexes chunk text
* FAISS artifacts map vectors back to `chunks.id`

Known implementation gaps / deferred items:

* full citation ingestion from `opinions_cited`
* section-aware chunk labels
* case-level grouping of chunk-level search results
...
* **v0.9** — updated implementation-status notes for the deployed MVP: schema is wired into the app and ingestion/chunking pipeline, chunking is currently paragraph-first with `section_hint = NULL`, citation table/API support exists but full citation ingestion is deferred, and stale “immediate next step” language was replaced with current implementation status.
